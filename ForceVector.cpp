...
ForceVector::ForceVector(Tuple& direction, double magnitude)
: Tuple()
{
    double x;
    double y;
    ...
    */
    ...
    //std::cout << "ForceVector constructor, input tuple: " << std::endl << direction;
...
std::ostream& operator<<(std::ostream& stream, const ForceVector& forceVector)
{
    stream << "ForceVector:" << std::endl;
    stream << *(dynamic_cast<const Tuple*>(&forceVector));
    double magnitude = forceVector.distance(Tuple(0,0,0));
